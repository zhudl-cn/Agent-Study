...
| **Loader** | 文档加载 | PDF / Markdown / Docx / PPT / HTML |
| **DataSource** | 数据源 | 本地文件 |
...
### 5.5 云端数据源模块设计 (Cloud Data Source Design)

针对 JIRA 和 Confluence 等云端数据源，设计统一的抓取、处理、索引模块，实现自动化数据同步与智能摘要生成。
...
│                                                                             │
│  ┌─────────────────┐    ┌─────────────────┐    ┌─────────────────┐          │
│  │   JIRA Client   │    │ Confluence Client│    │  其他云端源...   │          │
│  └────────┬────────┘    └────────┬────────┘    └────────┬────────┘          │
│           └──────────────────────┼──────────────────────┘                   │
│                                  ▼                                          │
│  ┌───────────────────────────────────────────────────────────────────────┐  │
│  │                         数据抓取调度器                                  │  │
│  │  • 全量/增量抓取策略    • 定时任务管理    • 断点续传                    │  │
│  └───────────────────────────────────────────────────────────────────────┘  │
│                                  ▼                                          │
│  ┌───────────────────────────────────────────────────────────────────────┐  │
│  │                         数据清洗与预处理                                │  │
│  │  • HTML/Confluence 格式解析    • 内容清洗    • 元数据提取              │  │
│  └───────────────────────────────────────────────────────────────────────┘  │
│                                  ▼                                          │
│  ┌───────────────────────────────────────────────────────────────────────┐  │
│  │                         BERT 摘要与目录生成                             │  │
...
│  ┌───────────────────────────────────────────────────────────────────────┐  │
│  │                         向量化与入库                                    │  │
│  │  • 复用现有 Embedding 流水线    • 双路索引（向量 + BM25）               │  │
│  └───────────────────────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
| 模块 | 职责 | 说明 |
|-----|------|------|
| **JIRA Client** | JIRA 数据抓取 | 获取项目、Issue、评论、附件 |
| **Confluence Client** | Confluence 数据抓取 | 获取空间、页面、子页面、附件 |
| **抓取调度器** | 同步策略管理 | 全量/增量抓取、定时任务、状态管理 |
| **数据清洗** | 内容预处理 | HTML 解析、格式转换、噪音去除 |
| **BERT 摘要** | 智能摘要生成 | 文档摘要、关键词提取 |
| **目录生成** | 层级索引构建 | 项目/空间目录树、元数据索引 |
| **向量化入库** | RAG 流水线对接 | 复用现有 Embedding 和存储模块 |

#### 5.5.3 调度策略

| 策略 | 说明 | 适用场景 |
|-----|------|------|
| **全量抓取** | 首次运行时抓取全部数据 | 初始化索引 |
| **增量抓取** | 基于 `lastModified` 时间戳抓取变更 | 日常更新 |
| **定时任务** | 通过 Cron 表达式定时执行 | 自动化维护 |

#### 5.5.4 BERT 摘要模型选型

| 模型 | 参数规模 | 语言支持 | 特点 |
|-----|---------|---------|------|
| `bert-extractive-summarizer` | Base | 多语言 | 抽取式摘要，快速 |
| `facebook/bart-large-cnn` | Large | 英文 | 生成式摘要，高质量 |
| `csebuetnlp/mT5_multilingual_XLSum` | Base | 多语言 | 多语言生成式摘要 |
...
├── JIRA/
│   ├── PROJECT-A/
│   │   ├── _index.json          # 项目元数据 + 摘要
│   │   └── issues/              # Issue 详情
│   └── PROJECT-B/
└── Confluence/
    ├── Space-A/
    │   ├── _index.json          # 空间元数据 + 摘要
    │   └── pages/               # 页面内容
    └── Space-B/
```
...
| 字段 | 说明 |
|-----|------|
| `source` | 数据源类型（jira/confluence） |
| `source_url` | 原始链接 |
| `project` / `space` | 所属项目/空间 |
| `summary` | BERT 生成的摘要 |
| `keywords` | 提取的关键词 |
| `last_synced` | 最后同步时间 |
...
| 任务 | 说明 | 验收标准 |
|-----|------|---------|
| B1：Loader 实现 ✅ | PDF/Markdown/Word/PPT 加载 | 文档可正确解析 |
...
### 阶段 G：JIRA/Confluence 自动化抓取与智能导入

| 任务 | 说明 | 验收标准 |
|-----|------|---------|
| G1：JIRA API 客户端 | 实现 JIRA REST API 客户端 | 可获取 Ticket 列表、详情、评论、附件 |
| G2：Confluence API 客户端 | 实现 Confluence REST API 客户端 | 可获取页面列表、内容、子页面、附件 |
| G3：数据抓取调度器 | 定时/增量抓取机制 | 支持定时任务和增量更新检测 |
| G4：BERT 摘要模型集成 | 集成 BERT 摘要生成模型 | 可对文档内容生成摘要 |
| G5：智能目录生成 | 基于内容结构和摘要生成目录树 | 目录层级清晰、可导航 |
| G6：数据清洗与预处理 | HTML/Markdown 解析与清洗 | 内容干净、结构化 |
| G7：向量化与入库 | 调用现有流水线导入向量库 | 文档可检索、来源可追溯 |
| G8：配置管理 | API 凭证、抓取规则配置 | 配置灵活、安全存储 |
| G9：CLI 工具封装 | 提供命令行工具触发抓取 | 可手动或脚本调用 |
