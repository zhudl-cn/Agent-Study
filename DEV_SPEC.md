...
RAG Assistant 支持文档中**图片内容的智能处理与检索**，采用 **Image-to-Text** 策略将图片转换为可检索的文本描述。
...
| 模型 | 参数规模 | 运行模式 | 推荐场景 |
|-----|---------|---------|---------|
| **PaddleOCR-VL-1.5** | 0.9B | CPU / GPU | **默认选择**（文档专用、SOTA） |
| **MiniCPM-V-2.6** | 3B | CPU / GPU | 通用图片理解 |
| **Qwen-VL-Chat** | 7B | GPU | 高精度需求 |

**推荐选型**：
- **文档场景**：`PaddleOCR-VL-1.5`（0.9B，文档解析 SOTA，表格/公式/印章）
- **通用场景**：`MiniCPM-V-2.6`（3B，多语言支持）
