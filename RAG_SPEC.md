# WorkForge Developer Specification
...
> **核心定位：开发者工作流的智能锻造坊**
> 
> WorkForge 是一个面向开发者的综合性 AI 助手，将文档检索、代码审查、项目管理、文件处理、日志分析等日常工作流程统一整合到一个智能平台中。它就像一个"数字锻造坊"，将分散的开发工具和信息源熔炼成高效、流畅的工作体验。

本项目基于 **RAG (Retrieval-Augmented Generation)** 技术设计，采用 **C/S 架构**：后端部署独立的 REST API 服务器，前端通过 **VS Code 扩展** 提供两种交互模式——独立的 **Webview 聊天界面** 或 **Copilot Chat 工具调用**。开发者可以在编辑器中无缝访问私有知识库、代码仓库、项目管理系统、系统日志等多种数据源。
...
└────────────────────────────────────┼────────────────────────────────────────┘
                                     ...
                                     ▼
┌─────────────────────────────────────────────────────────────────────────────┐
│                         WorkForge Server (FastAPI)                          │
│                                                                             │
│  ┌──────────────┐ ┌──────────────┐ ┌──────────────┐ ┌──────────────┐        │
│  │ 文档检索 API │ │ 代码审查 API │ │ JIRA 集成API │ │ Syslog分析API│        │
│  └──────────────┘ └──────────────┘ └──────────────┘ └──────────────┘        │
│  ┌──────────────┐ ┌──────────────┐ ┌──────────────────────────────┐         │
│  │ PR 管理 API  │ │ 格式转换 API │ │      RAG 检索引擎            │         │
│  └──────────────┘ └──────────────┘ └──────────────────────────────┘         │
└─────────────────────────────────────────────────────────────────────────────┘
```

### 核心能力域 (Core Capabilities)

WorkForge 覆盖开发者日常工作的六大核心场景：

| 能力域 | 功能描述 | 典型场景 |
|-------|---------|---------|
| 📚 **智能文档检索** | 支持本地与云端文档的语义检索，理解图片、表格等多模态内容 | 快速查找技术文档、API 规范、设计文档 |
| 💻 **代码审查助手** | 自动化 Code Review，识别潜在问题，提供改进建议 | PR 代码质量检查、最佳实践推荐 |
| 📋 **JIRA 智能管理** | 搜索、理解、辅助修改 JIRA Ticket 内容 | Ticket 内容补全、关联分析、状态跟踪 |
| 🔄 **文件格式转换** | 支持多种文档格式之间的智能转换 | Markdown ↔ PDF、代码片段提取 |
| 🔀 **PR 流程管理** | 管理 Pull Request 生命周期，自动化常见操作 | PR 创建、冲突检测、合并建议 |
| 🔍 **Syslog 智能分析** | 解析、搜索、关联分析系统日志，快速定位问题根因 | 错误日志检索、异常模式识别、故障排查 |

#### 1️⃣ 双模式交互 (Dual Interaction Modes)

WorkForge VS Code 扩展提供两种交互模式，满足不同使用偏好：
...
- 通过 `@workforge` 命令在 Copilot Chat 中调用
- 与 Copilot 对话无缝集成
- 适合快速查询和上下文相关问题
...
模式 A (Webview):                    模式 B (Chat Participant):
┌─────────────────────┐              ┌─────────────────────────────┐
│ WorkForge Chat      │              │ Copilot Chat                │
├─────────────────────┤              ├─────────────────────────────┤
│ 🤖 有什么可以帮您？  │              │ 用户: @workforge 搜索       │
│                     │              │       Kubernetes 部署文档   │
│ 👤 搜索 K8s 部署文档 │              │                             │
│                     │              │ 🤖 找到 3 篇相关文档:       │
...
#### 2️⃣ 多源融合 (Multi-Source Fusion)

WorkForge 不仅是一个 RAG 系统，更是一个**多源数据融合平台**：

- **本地文档**：PDF、Markdown、Word 等格式的技术文档
- **云端存储**：Confluence、Notion、Google Drive 等在线知识库
- **代码仓库**：GitHub/GitLab 代码库的语义理解
- **项目管理**：JIRA、Linear 等 Ticket 系统的结构化数据
- **系统日志**：Syslog、应用日志等运维数据的智能检索与分析
- **多模态内容**：图片、表格、流程图的智能识别与检索

#### 3️⃣ 开发者工作流覆盖 (Developer Workflow Coverage)

| 工作阶段 | WorkForge 能力 |
|---------|---------------|
| 📖 **学习理解** | 文档检索、代码解读、架构理解 |
| ✍️ **编码开发** | 代码片段搜索、最佳实践推荐、格式转换 |
| 🔍 **代码审查** | 自动化 Review、问题检测、改进建议 |
| 📋 **任务管理** | JIRA 搜索、Ticket 理解、内容辅助 |
| 🚀 **代码交付** | PR 管理、冲突检测、合并流程 |
| 🛠️ **故障排查** | Syslog 分析、错误定位、根因追溯 |

#### 4️⃣ 可扩展架构 (Extensible Architecture)

WorkForge 采用高度模块化的可插拔设计，便于扩展新的数据源和能力：

- **数据源可插拔**：轻松接入新的文档库、代码库、项目管理工具
...
WorkForge 采用**多阶段 RAG 架构**，通过"粗排召回 → 融合排序 → 精排重排"的流水线设计，平衡检索效率与回答质量。
...
WorkForge 采用**全链路可插拔设计**，核心组件均可通过配置切换，无需修改业务代码。
...
| **DataSource** | 数据源 | 本地文件 / JIRA / Syslog |
...
WorkForge 设计上支持 **MCP (Model Context Protocol)** 协议，可作为 MCP Server 被 GitHub Copilot、Claude Desktop 等 MCP Client 调用。
...
3. 通过 MCP 协议调用 WorkForge 能力

---

### 2.4 多模态图像处理 (Multimodal Image Processing)

WorkForge 支持文档中**图片内容的智能处理与检索**，采用 **Image-to-Text** 策略将图片转换为可检索的文本描述。

#### 2.4.1 处理策略

| 策略 | 说明 | 优势 |
|-----|------|------|
| **Image-to-Text** | 使用本地 Vision 模型生成图片描述 | 复用现有文本检索流程 |
| **图文关联** | 图片描述与上下文文本关联存储 | 提升检索相关性 |
| **多格式支持** | 支持 PNG、JPG、SVG 等常见格式 | 覆盖常见文档图片 |
...
| 模型 | 参数规模 | 运行模式 | 推荐场景 |
|-----|---------|---------|---------|
| **MiniCPM-V-2.6** | 3B | CPU / GPU | **默认选择**（轻量、多语言） |
| **Qwen-VL-Chat** | 7B | GPU | 高精度需求（英日支持） |
| **LLaVA-1.5** | 7B | GPU | 通用场景 |

**推荐选型**：
- **CPU 模式**：`MiniCPM-V-2.6`（量化版，适合纯 CPU 运行）
- **GPU 模式**：`Qwen-VL-Chat`（多语言支持，含日文）
...
| 类型 | 说明 | 处理方式 |
|-----|------|---------|
| **截图/照片** | 产品截图、系统界面 | Vision 模型描述 |
| **流程图/架构图** | 技术文档中的图表 | Vision 模型描述 |
| **代码截图** | 代码片段截图 | OCR + 描述 |
| **表格图片** | 表格截图 | OCR 转文本 |
...
WorkForge 内置**全链路追踪**与**RAG 质量评估**能力，支持系统透明化监控与持续优化。
...
WorkForge 设计上支持多种业务场景扩展，可适配不同的数据源、客户端和工作流。

#### 2.6.1 多数据源适配

| 数据源类型 | 说明 |
|----------|------|
| **本地文档** | PDF、Markdown、Word 等文档 |
| **Syslog 日志** | 系统日志结构化解析与检索 |
| **代码仓库** | GitHub/GitLab 代码检索 |
| **JIRA/项目管理** | Ticket 搜索与内容理解 |
| **Confluence/Wiki** | 知识库文档检索 |
...
| 场景 | 能力 | 说明 |
|-----|------|------|
| **文档检索** | 知识库问答 | 技术文档、规范、手册检索 |
| **代码审查** | Review 辅助 | 代码质量检测、改进建议 |
| **故障排查** | Syslog 分析 | 日志检索、错误定位、根因追溯 |
| **任务管理** | JIRA 集成 | Ticket 搜索、内容理解 |

#### 2.6.4 扩展路径

| 扩展方向 | 说明 |
|---------|------|
| **新增数据源** | 实现 DataSource 接口即可接入 |
| **新增客户端** | 通过 REST API 接入任意客户端 |
| **定制化场景** | 通过 Prompt 模板和评估指标定制 |
...
WorkForge Server 通过 REST API 向客户端提供 RAG 检索服务，使其能够作为知识上下文提供者，对接 VS Code 扩展和其他客户端。
...
| 模型 | 特点 | 硬件要求 |
|-----|------|---------|
| **MiniCPM-V-2.6** | 轻量级，适合 CPU/低端 GPU | ≥8GB RAM |
| **Qwen-VL-Chat** | 中文理解能力强 | ≥8GB VRAM |
| **LLaVA-1.5** | 通用性好 | ≥8GB VRAM |

> **注意**：所有 Vision 模型均为本地运行，不依赖外部 API。
...
| C4：多模态图片处理 | Vision 模型集成 | 图片可描述和检索 |
...
当前支持：PDF、Markdown、Word、HTML

**扩展方向**：
- **代码仓库**：集成代码解析器，支持代码语义检索
- **Syslog**：对接日志数据源，支持故障排查
- **Confluence/JIRA**：通过 API 集成企业知识库
